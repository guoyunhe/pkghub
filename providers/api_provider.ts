...
/**
 * Custom serializer for API responses that ensures consistent JSON structure across all API
 * endpoints. Wraps response data in a 'data' property and handles pagination metadata for Lucid ORM
 * query results.
 */
class ApiSerializer extends BaseSerializer<{
  ...
  /**
   * Wraps all serialized data under this key in the response object. Example: { data: [...] }
   * instead of returning raw arrays/objects
   */
  wrap: 'data' = 'data';

  /**
   * Validates and defines pagination metadata structure for paginated responses. Ensures that
   * pagination info from Lucid queries is properly formatted.
   *
...
/**
 * Adds the serialize method to all HttpContext instances. Usage in controllers: return
 * ctx.serialize(data) This ensures all API responses follow the same structure with data wrapping.
 */
HttpContext.instanceProperty('serialize', serialize);

/**
 * Module augmentation to add the serialize method to HttpContext. This allows controllers to use
 * ctx.serialize() for consistent API responses.
 */
